...
extension BatchesResponse: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    tasks = rawTasks.map { rawIndexName, rawTaskID in
      IndexedTask(
        indexName: .init(rawValue: rawIndexName), taskID: .init(rawValue: String(rawTaskID)))
    }
    objectIDs = try container.decode(forKey: .objectIDs)
  }

  public func encode(to encoder: Encoder) throws {
    var container = encoder.container(keyedBy: CodingKeys.self)
    let rawTasks = [String: String](
      uniqueKeysWithValues: tasks.map { ($0.indexName.rawValue, $0.taskID.rawValue) })
