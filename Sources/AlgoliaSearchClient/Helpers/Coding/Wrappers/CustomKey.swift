...
@propertyWrapper
public struct CustomKey<Key: Hashable & RawRepresentable, Value: Codable>: Codable
where Key.RawValue == String {
  ...
  public init(from decoder: Decoder) throws {
    ...
    for rawKey in container.allKeys {
      guard let key = Key(rawValue: rawKey.stringValue) else {
        throw DecodingError.dataCorruptedError(
          forKey: rawKey, in: container, debugDescription: "Cannot create \(Value.self) value")
      }
