...
class CustomClientConfigurationTests: XCTestCase {
  func testCustomHeaders() throws {
    ...
    requester.onRequest = { request in
      ...
      Logger.info("\(headers)")
      XCTAssert(
        headers.contains(where: { $0.key == "Default-Header" && $0.value == "default-header-value" }
        ))
      XCTAssert(
        (request.allHTTPHeaderFields ?? [:]).contains(where: {
          $0.key == "Another-Header" && $0.value == "another-value"
        }))
      exp.fulfill()
    }
