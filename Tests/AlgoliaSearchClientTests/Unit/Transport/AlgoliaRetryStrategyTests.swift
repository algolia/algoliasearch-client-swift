...
class AlgoliaRetryStrategyTests: XCTestCase {
  ...
  let retryableErrorResult = Result<String, Error>.failure(
    TransportError.requestError(URLError(.networkConnectionLost)))
  let timeoutErrorResult = Result<String, Error>.failure(
    TransportError.requestError(URLError(.timedOut)))
  ...
  func testResetExpired() {
    ...
    let strategy = AlgoliaRetryStrategy(
      hosts: [host3, host4, host5], hostsExpirationDelay: expirationDelay)
  ...
  func testLoad() throws {
    ...
    let queuesWithOpCount = (1...queueCount)
      .map {
        (queue: DispatchQueue(label: "queue\($0)"), operationCount: Int.random(in: 100...1000))
      }
    ...
    allOperationsFinishedExpectation.expectedFulfillmentCount = queuesWithOpCount.map(
      \.operationCount
    ).reduce(0, +)
