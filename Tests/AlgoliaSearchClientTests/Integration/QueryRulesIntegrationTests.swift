...
class QueryRulesIntegrationTests: IntegrationTestCase {
  override var indexNameSuffix: String? { "rules" }
  ...
  func queryRules() throws {
    ...
    let settings = Settings().set(
      \.attributesForFaceting, to: [.default("brand"), .default("model")])
    ...
    let brandAutomaticFacetingRule = Rule(objectID: "brand_automatic_faceting")
      ...
      .set(
        ...
        to: Rule.Consequence()
          .set(
            \.automaticFacetFilters, to: [.init(attribute: "brand", score: 42, isDisjunctive: true)]
          )
      )
      .set(
        \.validity,
        to: [
          .init(
            from: Date(timeIntervalSince1970: 1_532_439_300),
            until: Date(timeIntervalSince1970: 1_532_525_700)),
          .init(
            from: Date(timeIntervalSince1970: 1_532_612_100),
            until: Date(timeIntervalSince1970: 1_532_698_500)),
        ]
      )
      .set(
        \.description,
        to: "Automatic apply the faceting on `brand` if a brand value is found in the query")
    ...
    let queryEditsRule = Rule(objectID: "query_edits")
      .set(
        \.conditions,
        to: [.init(anchoring: .is, pattern: .literal("mobile phone"), alternatives: .true)]
      )
      .set(
        \.consequence,
        to: Rule.Consequence()
    ...
    let fetchedBrandAutomaticFacetingRule = try index.getRule(
      withID: brandAutomaticFacetingRule.objectID)
    ...
    try AssertEquallyEncoded(
      fetchedRules.first(where: { $0.objectID == brandAutomaticFacetingRule.objectID }),
      brandAutomaticFacetingRule)
    try AssertEquallyEncoded(
      fetchedRules.first(where: { $0.objectID == queryEditsRule.objectID }), queryEditsRule)
    try AssertEquallyEncoded(
      fetchedRules.first(where: { $0.objectID == queryPromoRule.objectID }), queryPromoRule)
    try AssertEquallyEncoded(
      fetchedRules.first(where: { $0.objectID == queryPromoSummerRule.objectID }),
      queryPromoSummerRule)

    try index.deleteRule(withID: brandAutomaticFacetingRule.objectID).wait()
    try AssertThrowsHTTPError(
      index.getRule(withID: brandAutomaticFacetingRule.objectID), statusCode: 404)
