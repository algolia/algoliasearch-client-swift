...
class MultipleOperationsIntegrationTests: IntegrationTestCase {
  override var indexNameSuffix: String? {
    "exists"
  }

  lazy var firstIndex: Index! = client?.index(withName: "multiple_operations")

  lazy var secondIndex: Index! = client?.index(withName: "multiple_operations_dev")
  ...
  func multipleOperations() throws {
    ...
    let results = try client.multipleQueries(
      queries: [
    ...
    let resultsStopIfEnough = try client.multipleQueries(
      queries: [
        .init(indexName: firstIndex.name, query: Query.empty.set(\.hitsPerPage, to: 2)),
        .init(indexName: secondIndex.name, query: Query.empty.set(\.hitsPerPage, to: 2)),
