...
class DisjunctiveFacetingHelperTests: XCTestCase {
  ...
  func testAppliedDisjunctiveFacetValues() {
    ...
    XCTAssertEqual(
      helper.appliedDisjunctiveFacetValues(for: "brand"), ["samsung", "sony", "apple"])
  }
  ...
  func testMergeFacetStats() throws {
    ...
    assertEqual(
      response.facetStats?["price"], FacetStats(min: 7, max: 103, avg: 55, sum: 3000),
      file: #filePath,
      line: #line)
    assertEqual(
      response.facetStats?["target"], FacetStats(min: 1, max: 10, avg: 5.5, sum: 43),
      file: #filePath,
      line: #line)
    assertEqual(
      response.facetStats?["size"], FacetStats(min: 20, max: 56, avg: 38, sum: 242),
      file: #filePath,
      line: #line)
    assertEqual(
      response.facetStats?["note"], FacetStats(min: 1, max: 5, avg: 3, sum: 37),
      file: #filePath,
      line: #line)
  }

  func assertEqual(
    _ lhs: FacetStats?, _ rhs: FacetStats?, file: StaticString = #filePath, line: UInt = #line
  ) {
    guard let lhs else {
