...
extension StringWrapper {
  static var random: Self {
    .init(stringLiteral: String.random(length: .random(in: 1...20)))
  }
}
...
class BrowseIntegrationTests: IntegrationTestCase {
  ...
  func browseRules() throws {
    let rules: [Rule] = (0...50).map { _ in
      Rule(objectID: .random)
        .set(
          \.conditions,
          to: [.init(anchoring: .is, pattern: .literal(.random(length: .random(in: 0...10))))]
        )
    ...
    let synonyms: [Synonym] = (0...50).map { _ in
      ...
      case 2:
        return .alternativeCorrection(
          objectID: .random, word: .random, corrections: [.random, .random, .random],
          typo: Bool.random() ? .one : .two)
      default:
        return .placeholder(
          objectID: .random, placeholder: "<token>", replacements: [.random, .random])
      }
    }
