...
class CopyIndexIntergrationTests: IntegrationTestCase {
  ...
  let rule = Rule(objectID: "company_auto_faceting")
    ...
    .set(
      \.consequence,
      to: Rule.Consequence().set(
        \.automaticFacetFilters, to: [Rule.AutomaticFacetFilters(attribute: "company")]))
  let synonym = Synonym.placeholder(
    objectID: "google_placeholder", placeholder: "<GOOG>", replacements: ["Google", "GOOG"])
