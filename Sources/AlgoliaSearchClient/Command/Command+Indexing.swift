...
extension Command {
  enum Indexing {
    struct SaveObject: AlgoliaCommand {
      ...
      init(indexName: IndexName, record: some Encodable, requestOptions: RequestOptions?) {
    ...
    struct GetObject: AlgoliaCommand {
      ...
      init(
        indexName: IndexName, objectID: ObjectID, attributesToRetrieve: [Attribute],
    ...
    struct ReplaceObject: AlgoliaCommand {
      ...
      init(
        indexName: IndexName, objectID: ObjectID, replacementObject record: some Encodable,
        requestOptions: RequestOptions?
      ) {
        self.requestOptions = requestOptions
        path = URL
    ...
    struct DeleteByQuery: AlgoliaCommand {
      ...
      init(
        indexName: IndexName, query: AlgoliaSearchClient.DeleteByQuery,
        requestOptions: RequestOptions?
      ) {
        self.requestOptions = requestOptions
        path = URL
        ...
        body = ParamsWrapper(query.urlEncodedString).httpBody
      }
    }
    ...
    struct PartialUpdate: AlgoliaCommand {
      ...
      init(
        indexName: IndexName, objectID: ObjectID, partialUpdate: AlgoliaSearchClient.PartialUpdate,
        createIfNotExists: Bool?, requestOptions: RequestOptions?
      ) {
        let requestOptions = requestOptions.updateOrCreate(
          {
            guard let createIfNotExists else { return [:] }
