...
class SearchCommandTests: XCTestCase, AlgoliaCommandTest {
  func testSearch() {
    let command = Command.Search.Search(
      indexName: test.indexName, query: test.query, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .read,
      method: .post,
  ...
  func testBrowseCursor() {
    let command = Command.Search.Browse(
      indexName: test.indexName, cursor: test.cursor, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .read,
      method: .post,
  ...
  func testSearchForFacets() {
    let command = Command.Search.SearchForFacets(
      ...
      query: nil,
      requestOptions: test.requestOptions)
    let body = ParamsWrapper(
      Query().set(\.customParameters, to: ["facetQuery": "test facet query"]).urlEncodedString
    ).httpBody
    check(
      command: command,
      callType: .read,
