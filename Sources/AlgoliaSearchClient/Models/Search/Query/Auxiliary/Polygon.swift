...
public struct Polygon: Equatable {
  ...
  public init(
    _ point1: (Double, Double), _ point2: (Double, Double), _ point3: (Double, Double),
...
extension Polygon: RawRepresentable {
  ...
  public init?(rawValue: [Double]) {
    ...
    if rawValue.count == 6 {
      ...
      tailPoints = stride(from: rawValue.startIndex.advanced(by: 6), to: rawValue.endIndex, by: 2)
        .map { Point(latitude: rawValue[$0], longitude: rawValue[$0 + 1]) }
    }

    self.init(
      .init(latitude: rawValue[0], longitude: rawValue[1]),
  ...
  public init(from decoder: Decoder) throws {
    ...
    guard let value = Polygon(rawValue: rawValue) else {
      throw DecodingError.dataCorruptedError(
        in: container,
        debugDescription:
          "Polygon may be constructed with at least 6 double values. \(rawValue.count) found")
    }
    self = value
  }
...
extension Polygon: URLEncodable {
  public var urlEncodedString: String {
    "[\(rawValue.map(\.urlEncodedString).joined(separator: ","))]"
  }
}
