...
class AdvancedCommandTests: XCTestCase, AlgoliaCommandTest {
  func testTaskStatus() {
    let command = Command.Advanced.TaskStatus(
      indexName: test.indexName, taskID: test.taskID, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .read,
      method: .get,
  ...
  func testGetLogs() {
    let command = Command.Advanced.GetLogs(
      indexName: test.indexName, offset: 10, length: 100, logType: .all,
      requestOptions: test.requestOptions)
