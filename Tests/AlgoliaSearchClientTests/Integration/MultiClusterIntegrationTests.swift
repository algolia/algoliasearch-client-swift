...
extension SearchClient {
  fileprivate func remove(_ userID: UserID) throws {
    while true {
      do {
  ...
  fileprivate func exists(_ userID: UserID) throws -> Bool {
    do {
...
class MultipleClusterIntegrationTests: IntegrationTestCase {
  ...
  override var allowFailure: Bool { true }
  ...
  func userID(_ id: String) -> UserID {
    UserID(rawValue: TestIdentifier(date: date, suffix: id).rawValue)
  }
