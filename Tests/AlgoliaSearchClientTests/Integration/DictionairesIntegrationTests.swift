...
class DictionairesIntegrationTests: IntegrationTestCase {
  ...
  override func setUpWithError() throws {
    let fetchedCredentials = Result(catching: { try TestCredentials(environment: .secondary) })
      .mapError { XCTSkip("\($0)") }
  ...
  func stopwordsDictionary() throws {
    ...
    var response = try client.searchDictionaryEntries(
      in: StopwordsDictionary.self, query: "\(entryID)")
    ...
    let stopwordEntry = StopwordsDictionary.Entry(
      objectID: entryID, language: .english, word: "down", state: .enabled)
    try client.saveDictionaryEntries(
      to: StopwordsDictionary.self, dictionaryEntries: [stopwordEntry]
    ).wait()
    ...
    try client.deleteDictionaryEntries(from: StopwordsDictionary.self, withObjectIDs: [entryID])
      .wait()
    ...
    let previousEntries = try client.searchDictionaryEntries(
      in: StopwordsDictionary.self, query: ""
    ).hits
    ...
    let settings = DictionarySettings(
      disableStandardEntries: .init(stopwords: [.english: true, .french: true]))
  ...
  func compoundsDictionary() throws {
    ...
    var response = try client.searchDictionaryEntries(
      in: CompoundsDictionary.self, query: "\(entryID)")
    XCTAssert(response.hits.isEmpty)

    // Add the following entry in the `compounds` dictionary with **save_dictionary_entries** and wait for the task to finish
    let compoundEntry = CompoundsDictionary.Entry(
      objectID: entryID, language: .german, word: "kopfschmerztablette",
      decomposition: ["kopf", "schmerz", "tablette"])
    try client.saveDictionaryEntries(
      to: CompoundsDictionary.self, dictionaryEntries: [compoundEntry]
    ).wait()
    ...
    // Delete the entry with **delete_dictionary_entries** and wait for the task to finish
    try client.deleteDictionaryEntries(from: CompoundsDictionary.self, withObjectIDs: [entryID])
      .wait()
  ...
  func pluralsDictionary() throws {
    ...
    var response = try client.searchDictionaryEntries(
      in: PluralsDictionary.self, query: "\(entryID)")
    XCTAssert(response.hits.isEmpty)

    // Add the following entry in the `plurals` dictionary with **save_dictionary_entries** and wait for the task to finish
    let pluralEntry = PluralsDictionary.Entry(
      objectID: entryID, language: .french, words: ["cheval", "chevaux"])
    try client.saveDictionaryEntries(to: PluralsDictionary.self, dictionaryEntries: [pluralEntry])
      .wait()
    ...
    // Delete the entry with **delete_dictionary_entries** and wait for the task to finish
    try client.deleteDictionaryEntries(from: PluralsDictionary.self, withObjectIDs: [entryID])
      .wait()
