...
class IndexingIntegrationTests: IntegrationTestCase {
  ...
  func indexing() throws {
    ...
    let objectWithGeneratedIDCreation = try index.saveObject(
      objectWithGeneratedID, autoGeneratingObjectID: true)
    ...
    let objectsIDs: [ObjectID] = [
      .init(rawValue: .init(randomWithLength: 10)), .init(rawValue: .init(randomWithLength: 10)),
    ]
    ...
    let objectsWithGeneratedIDCreation = try index.saveObjects(
      objectsWithGeneratedID, autoGeneratingObjectID: true)
    try objectWithGeneratedIDCreation.wait()

    let generatedObjectIDs = objectsWithGeneratedIDCreation.batchesResponse.objectIDs.compactMap {
      $0
    }
    objectsWithGeneratedID = zip(objectsWithGeneratedID, generatedObjectIDs).map {
      $0.0.set(\.objectID, to: $0.1)
    }
    ...
    let fetchedObjects: [TestRecord] = try index.getObjects(withIDs: firstObjectsIDs).results
      .compactMap { $0 }
    ...
    let expectedRecords =
      [object, objectWithGeneratedID] + objects + objectsWithGeneratedID + batchRecords
    ...
    try index.partialUpdateObject(
      withID: objectID, with: .update(attribute: "string", value: "partiallyUpdated"),
      createIfNotExists: false
    ).wait()

    // Alter 2 records with partialUpdateObjects and collect taskID/objectID
    try index.partialUpdateObjects(
      updates: [
        (objectsIDs[0], .update(attribute: "string", value: "partiallyUpdated")),
        (objectsIDs[1], .increment(attribute: "numeric", value: 10)),
      ], createIfNotExists: false
    ).wait()

    // Wait for all collected tasks to terminate with waitTask
    // Retrieve all the previously altered records with getObject and check their content against the modified records
    let updated: [TestRecord] = try index.getObjects(withIDs: [objectID] + objectsIDs).results
      .compactMap { $0 }

    XCTAssertEqual(
      updated.first { $0.objectID == objectID }, object.set(\.string, to: "partiallyUpdated"))
    XCTAssertEqual(
      updated.first { $0.objectID == objectsIDs.first! },
      objects.first!.set(\.string, to: "partiallyUpdated"))
    XCTAssertEqual(
      updated.first { $0.objectID == objectsIDs.last! },
      objects.last!.set(\.numeric, to: objects.last!.numeric + 10))
  ...
  func saveGetObjectCallback() throws {
    ...
    let expectation = expectation(description: "Save-Wait-Create")

    index.saveObject(
      object, autoGeneratingObjectID: true,
      completion: extract { creation in
        creation.wait(
          completion: extract { _ in
            self.index.getObject(
              withID: creation.task.objectID,
              completion: extract { (fetchedObject: JSON) in
                XCTAssertEqual(fetchedObject["testField1"], "testValue1")
                XCTAssertEqual(fetchedObject["testField2"], 2)
                XCTAssertEqual(fetchedObject["testField3"], true)
                expectation.fulfill()
