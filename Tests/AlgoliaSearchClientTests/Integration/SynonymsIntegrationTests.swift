...
class SynonymsIntegrationTests: IntegrationTestCase {
  override var indexNameSuffix: String? { "synonyms" }
  ...
  func synonyms() throws {
    ...
    let multiWaySynonym: Synonym = .multiWay(
      objectID: "gba", synonyms: ["gba", "gameboy advance", "game boy advance"])

    try index.saveSynonym(multiWaySynonym).wait()
    let oneWaySynonym: Synonym = .oneWay(
      objectID: "wii_to_wii_u", input: "wii", synonyms: ["wii U"])
    let placeholderSynonym: Synonym = .placeholder(
      objectID: "playstation_version_placeholder", placeholder: "<PLAYSTATIONVERSION>",
      replacements: ["1", "One", "2", "3", "4", "4 Pro"])
    let altCorrection1Synonym: Synonym = .alternativeCorrection(
      objectID: "ps4", word: "ps4", corrections: ["playstation4"], typo: .one)
    let altCorrection2Synonym: Synonym = .alternativeCorrection(
      objectID: "psone", word: "psone", corrections: ["playstationone"], typo: .two)
    ...
    try AssertEquallyEncoded(
      fetchedSynonyms.first(where: { $0.objectID == multiWaySynonym.objectID }), multiWaySynonym)
    try AssertEquallyEncoded(
      fetchedSynonyms.first(where: { $0.objectID == oneWaySynonym.objectID }), oneWaySynonym)
    try AssertEquallyEncoded(
      fetchedSynonyms.first(where: { $0.objectID == placeholderSynonym.objectID }),
      placeholderSynonym)
    try AssertEquallyEncoded(
      fetchedSynonyms.first(where: { $0.objectID == altCorrection1Synonym.objectID }),
      altCorrection1Synonym)
    try AssertEquallyEncoded(
      fetchedSynonyms.first(where: { $0.objectID == altCorrection2Synonym.objectID }),
      altCorrection2Synonym)
    ...
    try AssertThrowsHTTPError(
      index.getSynonym(withID: placeholderSynonym.objectID), statusCode: 404)
    try AssertThrowsHTTPError(
      index.getSynonym(withID: altCorrection1Synonym.objectID), statusCode: 404)
    try AssertThrowsHTTPError(
      index.getSynonym(withID: altCorrection2Synonym.objectID), statusCode: 404)
