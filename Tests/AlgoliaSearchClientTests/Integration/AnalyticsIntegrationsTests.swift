...
class AnalyticsIntegrationTests: IntegrationTestCase {
  ...
  func abTesting() throws {
    ...
    XCTAssertEqual(
      fetchedABTest.endAt.timeIntervalSinceReferenceDate,
      abTest.endAt.timeIntervalSinceReferenceDate, accuracy: 1)
    ...
    try AssertThrowsHTTPError(
      try analyticsClient.getABTest(withID: creation.wrapped.abTestID), statusCode: 404)
  }
  ...
  func aaTesting() throws {
    ...
    let abTest = ABTest(
      ...
      variantB: .init(
        indexName: index.name, trafficPercentage: 10,
        customSearchParameters: Query().set(\.ignorePlurals, to: true))
    )
