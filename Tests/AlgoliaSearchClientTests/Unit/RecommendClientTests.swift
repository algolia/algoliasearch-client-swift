...
class RecommendClientTests: XCTestCase {
  func testRecommendations() {
    ...
    let client = RecommendClient(
      configuration: .init(applicationID: "test-app-id", apiKey: "test-api-key"),
      requester: requester)
    ...
    requester.onRequest = { request in
      XCTAssertEqual(
        request.url?.absoluteString.starts(
          with: "https://test-app-id-dsn.algolia.net/1/indexes/*/recommendations?"), true)
    ...
    client.getRecommendations(options: [
      .init(
        indexName: "IndexName",
  ...
  func testFrequentlyBoughtTogether() {
    ...
    client.getFrequentlyBoughtTogether(options: [
      .init(indexName: "IndexName", objectID: "B018APC4LE")
    ]) { _ in }
  ...
  func testRelatedProducts() {
    ...
    client.getRelatedProducts(options: [.init(indexName: "IndexName", objectID: "B018APC4LE")]) {
      _ in
    }
