...
public struct SearchResponse: Codable, JSONEncodable, Hashable {
    static let abTestVariantIDRule = NumericRule<Int>(minimum: 1, exclusiveMinimum: false, maximum: nil, exclusiveMaximum: false, multipleOf: nil)
    static let aroundLatLngRule = StringRule(minLength: nil, maxLength: nil, pattern: "^(-?\\d+(\\.\\d+)?),\\s*(-?\\d+(\\.\\d+)?)$")
    ...
    public init(abTestID: Int? = nil, abTestVariantID: Int? = nil, aroundLatLng: String? = nil, automaticRadius: String? = nil, exhaustive: Exhaustive? = nil, exhaustiveFacetsCount: Bool? = nil, exhaustiveNbHits: Bool? = nil, exhaustiveTypo: Bool? = nil, facets: [String: [String: Int]]? = nil, facetsStats: [String: FacetsStats]? = nil, hitsPerPage: Int, index: String? = nil, indexUsed: String? = nil, message: String? = nil, nbHits: Int, nbPages: Int, nbSortedHits: Int? = nil, page: Int, parsedQuery: String? = nil, processingTimeMS: Int, processingTimingsMS: AnyCodable? = nil, queryAfterRemoval: String? = nil, redirect: Redirect? = nil, renderingContent: RenderingContent? = nil, serverTimeMS: Int? = nil, serverUsed: String? = nil, userData: AnyCodable? = nil, hits: [Hit], query: String, params: String) {
    ...
    public init(from dictionary: [String: AnyCodable]) throws {
        abTestID = dictionary["abTestID"]?.value as? Int

        abTestVariantID = dictionary["abTestVariantID"]?.value as? Int

        aroundLatLng = dictionary["aroundLatLng"]?.value as? String

        automaticRadius = dictionary["automaticRadius"]?.value as? String

        exhaustive = dictionary["exhaustive"]?.value as? Exhaustive

        exhaustiveFacetsCount = dictionary["exhaustiveFacetsCount"]?.value as? Bool

        exhaustiveNbHits = dictionary["exhaustiveNbHits"]?.value as? Bool

        exhaustiveTypo = dictionary["exhaustiveTypo"]?.value as? Bool

        facets = dictionary["facets"]?.value as? [String: [String: Int]]

        facetsStats = dictionary["facetsStats"]?.value as? [String: FacetsStats]

        guard let hitsPerPage = dictionary["hitsPerPage"]?.value as? Int else {
            throw GenericError(description: "Failed to cast")
        }
        self.hitsPerPage = hitsPerPage
        index = dictionary["index"]?.value as? String

        indexUsed = dictionary["indexUsed"]?.value as? String
        ...
        self.nbPages = nbPages
        nbSortedHits = dictionary["nbSortedHits"]?.value as? Int

        guard let page = dictionary["page"]?.value as? Int else {
            throw GenericError(description: "Failed to cast")
        }
        self.page = page
        parsedQuery = dictionary["parsedQuery"]?.value as? String

        guard let processingTimeMS = dictionary["processingTimeMS"]?.value as? Int else {
            throw GenericError(description: "Failed to cast")
        }
        self.processingTimeMS = processingTimeMS
        processingTimingsMS = dictionary["processingTimingsMS"]?.value as? AnyCodable

        queryAfterRemoval = dictionary["queryAfterRemoval"]?.value as? String

        redirect = dictionary["redirect"]?.value as? Redirect

        renderingContent = dictionary["renderingContent"]?.value as? RenderingContent

        serverTimeMS = dictionary["serverTimeMS"]?.value as? Int

        serverUsed = dictionary["serverUsed"]?.value as? String

        userData = dictionary["userData"]?.value as? AnyCodable
    ...
    public init(from decoder: Decoder) throws {
        ...
        processingTimingsMS = try container.decodeIfPresent(AnyCodable.self, forKey: .processingTimingsMS)
