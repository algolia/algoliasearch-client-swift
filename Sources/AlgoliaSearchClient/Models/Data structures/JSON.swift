...
extension JSON {
  public init(_ jsonRepresentable: some JSONRepresentable) {
    self = jsonRepresentable.json
  }

  public init<T: RawRepresentable>(_ rawJsonRepresentable: T) where T.RawValue: JSONRepresentable {
    self = rawJsonRepresentable.rawValue.json
  }

  public init<S: Sequence>(jsonSequence: S) where S.Element: JSONRepresentable {
...
extension [String: Any] {
  public init?(_ json: JSON) {
    guard case .dictionary(let dict) = json else {
      return nil
    }
    var resultDictionary = [String: Any]()
...
extension Numeric where Self: JSONRepresentable, Self: BinaryInteger {
  public var json: JSON { .number(Double(self)) }
}

extension FloatingPoint where Self: JSONRepresentable, Self: BinaryFloatingPoint {
  public var json: JSON { .number(Double(self)) }
}
