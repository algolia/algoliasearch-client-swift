...
class InsightsIntegrationTests: IntegrationTestCase {
  ...
  func insights() throws {
    ...
    let event = try InsightsEvent.click(
      name: "foo", indexName: index.name, userToken: "bar", timestamp: timestamp,
      objectIDs: ["one", "two"])
    ...
    let eventsAfterSearch = [
      try InsightsEvent.view(
        name: "foo", indexName: index.name, userToken: "bar", filters: ["filter:foo", "filter:bar"]),
      try InsightsEvent.view(
        name: "foo", indexName: index.name, userToken: "bar", objectIDs: ["one", "two"]),
      try InsightsEvent.click(
        name: "foo", indexName: index.name, userToken: "bar", queryID: queryID!,
        objectIDsWithPositions: [("one", 1), ("two", 2)]),
      try InsightsEvent.click(
        name: "foo", indexName: index.name, userToken: "bar", filters: ["filter:foo", "filter:bar"]),
      try InsightsEvent.click(
        name: "foo", indexName: index.name, userToken: "bar", objectIDs: ["one", "two"]),
      try InsightsEvent.conversion(
        name: "foo", indexName: index.name, userToken: "bar", queryID: nil,
        objectIDs: ["one", "two"]),
      try InsightsEvent.conversion(
        name: "foo", indexName: index.name, userToken: "bar", queryID: nil,
