...
class HTTPTransport: Transport {
  ...
  init(
    requestBuilder: HTTPRequestBuilder, operationLauncher: OperationLauncher,
  ...
  convenience init(
    requester: HTTPRequester, configuration: Configuration, retryStrategy: RetryStrategy,
    credentials: Credentials, operationLauncher: OperationLauncher
  ) {
    let requestBuilder = HTTPRequestBuilder(
      requester: requester, retryStrategy: retryStrategy, configuration: configuration,
      credentials: credentials)
    self.init(
      requestBuilder: requestBuilder, operationLauncher: operationLauncher, credentials: credentials
    )
  }

  func execute<Response: Decodable, Output>(
    _ command: some AlgoliaCommand, transform: @escaping (Response) -> Output,
    completion: @escaping (Result<Output, Swift.Error>) -> Void
  ...
  func execute<Response: Decodable, Output>(
    ...
    let request = requestBuilder.build(
      for: command, transform: transform, responseType: Output.self)
