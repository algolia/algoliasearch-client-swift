...
class IndexCommandTests: XCTestCase, AlgoliaCommandTest {
  ...
  func testOperationMove() {
    let operation: IndexOperation = .init(
      action: .move, destination: test.anotherIndexName, scopes: nil)
    let command = Command.Index.Operation(
      indexName: test.indexName, operation: operation, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .write,
      method: .post,
  ...
  func testOperationCopyAll() {
    let operation: IndexOperation = .init(
      action: .copy, destination: test.anotherIndexName, scopes: nil)
    let command = Command.Index.Operation(
      indexName: test.indexName, operation: operation, requestOptions: test.requestOptions)
    check(
      command: command,
  ...
  func testOperationCopyAllScopes() {
    let operation: IndexOperation = .init(
      action: .copy, destination: test.anotherIndexName, scopes: [.rules, .settings, .synonyms])
    let command = Command.Index.Operation(
      indexName: test.indexName, operation: operation, requestOptions: test.requestOptions)
    check(
      command: command,
