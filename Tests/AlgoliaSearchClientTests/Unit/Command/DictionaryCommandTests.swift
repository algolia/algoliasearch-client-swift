...
class DictionaryCommandsTests: XCTestCase, AlgoliaCommandTest {
  func testBatch() {
    ...
    let command = Command.Dictionaries.Batch(
      dictionary: StopwordsDictionary.self,
      requests: requests,
      clearExistingDictionaryEntries: true,
      requestOptions: test.requestOptions)
    let payload = Command.Dictionaries.Batch.Payload(
      requests: requests,
      clearExistingDictionaryEntries: true)
    check(
      command: command,
  ...
  func testSetSettings() {
    ...
    let command = Command.Dictionaries.SetSettings(
      settings: settings, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .write,
      method: .put,
