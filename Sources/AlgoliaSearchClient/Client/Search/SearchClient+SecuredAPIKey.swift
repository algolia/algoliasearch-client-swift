...
extension SearchClient {
  public func generateSecuredApiKey(
    parentApiKey: APIKey,
  ...
  public func getSecuredApiKeyRemainingValidity(_ securedAPIKey: APIKey) -> TimeInterval? {
    guard let rawDecodedAPIKey = securedAPIKey.rawValue.fromBase64() else { return nil }
    let prefix = "validUntil="
    guard let range = rawDecodedAPIKey.range(of: "\(prefix)\\d+", options: .regularExpression)
    else { return nil }
