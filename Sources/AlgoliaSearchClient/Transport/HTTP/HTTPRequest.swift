...
class HTTPRequest<ResponseType: Decodable, Output>: AsyncOperation, ResultContainer, TransportTask {
  ...
  var underlyingTask: TransportTask? {
    didSet {
      guard let task = underlyingTask else { return }
  ...
  var progress: Progress {
    underlyingTask?.progress ?? Progress(totalUnitCount: 1)
  }
  ...
  init(
    ...
    underlyingTask = nil
    hostIterator = retryStrategy.retryableHosts(for: callType)
  }
