...
extension URL {
  fileprivate static var template = URL(string: "www.algolia.com")!
}
...
class HTTPRequestTests: XCTestCase {
  class TestRequester: HTTPRequester {
    ...
    func perform<T>(request _: URLRequest, completion: @escaping (Result<T, Error>) -> Void)
      -> TransportTask where T: Decodable
    {
      ...
      case 4:
        completion(
          .failure(TransportError.requestError(URLError(.backgroundSessionWasDisconnected))))
      default:
        break
      }
  ...
  func testRetry() {
    ...
    let hosts = (1...3).map {
      RetryableHost(url: URL(string: "algolia\($0).com")!, callType: .read)
    }
