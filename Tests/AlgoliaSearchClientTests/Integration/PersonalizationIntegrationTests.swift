...
class PersonalizationIntegrationTests: IntegrationTestCase {
  ...
  func getStrategy() throws {
    let recommendationClient = PersonalizationClient(
      appID: client.applicationID, apiKey: client.apiKey, region: .custom("us"))
  ...
  func setStrategy() throws {
    ...
    } catch let .httpError(httpError) as TransportError
      where httpError.statusCode == HTTPStatusСode.tooManyRequests
    {
