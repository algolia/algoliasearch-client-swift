...
class SecuredAPIKeyRestrictionTests: XCTestCase {
  func testQuery() {
    ...
    XCTAssertEqual(
      restriction.urlEncodedString,
      "query=testQuery&clickAnalytics=true&restrictIndices=index1,index2&restrictSources=127.0.0.1,127.0.0.2&userToken=testUserToken&validUntil=\(Int(validUntil))"
    )
  }
}
