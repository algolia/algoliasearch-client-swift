...
class SecuredAPIKeysIntegrationTests: IntegrationTestCase {
  ...
  func securedAPIKeys() throws {
    ...
    let generatedAPIKey = client.generateSecuredApiKey(
      parentApiKey: parentAPIKey, with: restriction)
    ...
    try AssertThrowsHTTPError(
      try newClient.index(withName: index2.name).search(query: ""), statusCode: 403)
  }

  func testNotExpiredKey() throws {
    let restriction = SecuredAPIKeyRestriction().set(
      \.validUntil, to: Date().addingTimeInterval(.minutes(10)).timeIntervalSince1970)
