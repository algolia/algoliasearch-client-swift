...
protocol AlgoliaCommandTest {
  ...
  func check<Command: AlgoliaCommand>(
    command: Command, callType: CallType, method: HTTPMethod, urlPath: String,
    queryItems: Set<URLQueryItem>, body: Data?, additionalHeaders: [HTTPHeaderKey: String]?,
    requestOptions: RequestOptions, file: StaticString, line: UInt)
}
...
extension AlgoliaCommandTest {
  ...
  func check(
    command: some AlgoliaCommand, callType: CallType, method: HTTPMethod, urlPath: String,
    queryItems: Set<URLQueryItem>, body: Data?, additionalHeaders: [HTTPHeaderKey: String]? = nil,
    requestOptions: RequestOptions, file: StaticString = #file, line: UInt = #line
  ) {
    ...
    let requestQueryItems =
      URLComponents(url: request.url!, resolvingAgainstBaseURL: false)?.queryItems ?? []

    #if canImport(FoundationNetworking)
      XCTAssertEqual(
        request.allHTTPHeaderFields?.mapKeys { $0.lowercased() },
        requestOptions.headers.merging(additionalHeaders ?? [:]).mapKeys {
          $0.rawValue.lowercased()
        }, file: file, line: line)

      for item in queryItems {
        XCTAssertTrue(
          requestQueryItems.contains(where: {
            $0.name.lowercased() == item.name.lowercased() && $0.value == item.value
          }), file: file, line: line)
      }
      ...
      try XCTAssertEqual(
        request.httpBody.flatMap(dataToJSON), body.flatMap(dataToJSON), file: file, line: line)
    #else
      XCTAssertEqual(
        request.allHTTPHeaderFields,
        requestOptions.headers.merging(additionalHeaders ?? [:]).mapKeys { $0.rawValue },
        file: file, line: line)
