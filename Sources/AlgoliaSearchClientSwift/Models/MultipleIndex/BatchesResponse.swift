...
extension BatchesResponse: Codable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    let rawTasks: [String: String] = try container.decode(forKey: .tasks)
    self.tasks = rawTasks.map { rawIndexName, rawTaskID in TaskIndex(indexName: .init(rawValue: rawIndexName), taskID: .init(rawValue: rawTaskID)) }
