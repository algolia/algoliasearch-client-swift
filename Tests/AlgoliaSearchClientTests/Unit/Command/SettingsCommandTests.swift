...
class SettingsCommandTests: XCTestCase, AlgoliaCommandTest {
  func testGetSettings() {
    let command = Command.Settings.GetSettings(
  ...
  func testSetSettings() {
    let command = Command.Settings.SetSettings(
      indexName: test.indexName, settings: test.settings, resetToDefault: [.attributesToSnippet],
      forwardToReplicas: false, requestOptions: test.requestOptions)
    check(
      command: command,
      callType: .write,
      method: .put,
