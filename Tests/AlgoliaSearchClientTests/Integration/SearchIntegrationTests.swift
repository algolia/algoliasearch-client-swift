...
class SearchIntegrationTests: IntegrationTestCase {
  ...
  func search() throws {
    ...
    var foundObject = try index.findObject(
      matching: { (_: JSON) in false }, for: "", paginate: true, requestOptions: .none)
    XCTAssertNil(foundObject)

    foundObject = try index.findObject(
      matching: { (_: JSON) in true }, for: "", paginate: true, requestOptions: .none)
    XCTAssertEqual(foundObject?.page, 0)
    XCTAssertEqual(foundObject?.position, 0)

    foundObject = try index.findObject(
      matching: { (value: JSON) in value["company"] == "Apple" }, for: "algolia", paginate: true,
      requestOptions: .none)
    XCTAssertNil(foundObject)

    foundObject = try index.findObject(
      matching: { (value: JSON) in value["company"] == "Apple" },
      for: Query.empty.set(\.hitsPerPage, to: 5), paginate: false, requestOptions: .none)
    XCTAssertNil(foundObject)

    foundObject = try index.findObject(
      matching: { (value: JSON) in value["company"] == "Apple" },
      for: Query.empty.set(\.hitsPerPage, to: 5), paginate: true, requestOptions: .none)
    ...
    results = try index.search(
      query: ("elon" as Query).set(\.facets, to: ["*"]).set(\.filters, to: "company:tesla"))
    XCTAssertEqual(results.nbHits, 1)

    results = try index.search(
      query: ("elon" as Query).set(\.facets, to: ["*"]).set(
        \.filters, to: "(company:tesla OR company:spacex)"))
    XCTAssertEqual(results.nbHits, 2)

    let facetResults = try index.searchForFacetValues(of: "company", matching: "a")
    XCTAssertEqual(
      Set(facetResults.facetHits.map(\.value)), ["Algolia", "Amazon", "Apple", "Arista Networks"])
  }
}
